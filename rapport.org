...
:END:
 ...
 #+LaTeX: \tableofcontents\listoffigures\pagenumbering{arabic}\setcounter{page}{1}
* Introduction
...
d'apprentissage η; et ceci jusqu'à converger en un minimum.  
...
Attention à taille de η: plus il est grand plus on avance loin à
chaque pas donc plus on réduit théoriquement les itérations, mais si η
...
divergent. En revanche plus η est petit plus on avance lentement, mais
...
échantillon. L est la fonction de densité à paramètres θ correspondant
à un échantillon de variables aléatoires discrètes. Soit:
$L(θ)=p(x1...xN | θ)$ représentant la probabilité d'avoir les
observations x1...xN étant donné les paramètres θ. Les estimateurs θ̂
du maximum de vraisemblance des paramètres θ sont les valeurs
...
probabilité d'un échantillon en trouvant les valeurs des paramètres θ
...
\begin{figure}[!h]
\includegraphics[width=\textwidth]{./images/alpha_on_theta_ellipse.png}
