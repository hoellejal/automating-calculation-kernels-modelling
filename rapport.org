...
%  \includegraphics[height=1.4cm]{logos/Logo-Inria.pdf}
  ...
  \end{center}
   ...
   \Large Jury:
   \begin{itemize}
   \item Olivier Richard, Maître de Conférence UGA
   \item ???
   \item ???
   \end{itemize}
...
** Elaboration de modèles
   ...
   $duration \sim N(\alpha_1 \cdot mnk + \alpha_2 \cdot mn + \alpha_3 \cdot mk + \alpha_4 \cdot nk + \beta, \gamma_1 \cdot mnk + \gamma_2 \cdot mn + \gamma_3 \cdot mk + \gamma_4 \cdot nk + \delta)$ 
   ...
   $duration_i \sim N (\alpha_i*mnk_i+\beta_i, \gamma_i*mnk_i+\delta_i)$
   ...
   paramètres): $\alpha_i \sim N (\mu_\alpha , \sigma_a)$
   ...
   Dans ce modèle hiérarchique, on dira que $\mu_\alpha \sim N (\alpha_moy , \alpha_sd)$ où
   \alpha_moy et \alpha_sd sont les priors et $\sigma_\alpha \sim N (0,1)$.
