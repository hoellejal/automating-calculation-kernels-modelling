...
distribution.
***** A test with a higher sigma (needs a rework)
#+begin_src R :results output :session *R* :exports both
mu = c(-7.65489, 6.2369)
...
**** STARTED A hierarchical M_H-2 N_H-2 model
:LOGBOOK:
...
sm = stan_model(model_code = modelString)
#+end_src
**** TODO The priors for the hierarchical model
