...
Malheureusement l'exécution du sampling est beaucoup trop longue, ce
qui nous indique un problème dans le modèle.
*** 2019-06-11 tuesday
...
**** STARTED Plotting the posterior (with a M_H-1 N_H-2 model)
#+begin_src R :results output :session *R* :exports both
library(rstan)
library(dplyr)
...
#+begin_src R :results output :session *R* :exports both
extracted=rstan::extract(fit)
dtf=data.frame(intercept=extracted$intercept,mu=extracted$mu)
ggplot(dtf,aes(intercept.1,mu.1))

ggplot(data=dtf,mapping=aes(intercept.1,mu.1))+stat_ellipse(mapping=aes(intercept.1,mu.1))+geom_point(colour="red",alpha=0.2)+stat_ellipse(mapping=aes(intercept.26,mu.26))+geom_point(data=dtf,mapping=aes(intercept.26,mu.26),alpha=0.2,colour="blue")
#+end_src

#+RESULTS:
:  
: Error in stat_elipse() : impossible de trouver la fonction "stat_elipse"
