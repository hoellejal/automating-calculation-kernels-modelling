...
#+TAGS: PAPER(P) VIDEO(V) MEETING(m) REPORT(V)
...
*** 2019-05-09 thursday
**** Continued to read the book, notes on May 3rd section
**** Continued watching the MOOC, particularly the first and fourth modules
...
**** Tried to use Stan with more complex examples :STAN:R:
***** With a full polynomial model (needs a rework):
...
df$x2=df$x^2 #Need to add it here because we cannot in the stan model
head(df)
#+end_src
...
:    x            y   x2
: 1 13   729.215216  169
: 2 60  7439.417049 3600
: 3 83 14208.692665 6889
: 4 12   216.362362  144
: 5  2    -3.035517    4
: 6 35  2590.543016 1225
...
#+begin_src R :results output :session *R* :exports both
data = list(N=nrow(df),x=df$x,x2=df$x2,y=df$y)
fit = sampling(sm,data=data, iter=500, chains=8)
print(fit)
#+end_src
...
intercept       121.55    1.24 20.75    81.20   107.04   121.62   136.34
first_degree      4.75    0.05  0.94     2.93     4.08     4.75     5.40
second_degree     2.01    0.00  0.01     2.00     2.01     2.01     2.02
sigma           150.62    0.14  4.76   141.12   147.41   150.53   154.02
lp__          -2751.30    0.06  1.45 -2755.14 -2752.01 -2750.98 -2750.25
                 97.5% n_eff Rhat
intercept       159.41   278 1.02
first_degree      6.56   328 1.02
second_degree     2.03   462 1.01
sigma           159.71  1108 1.01
lp__          -2749.49   567 1.01

Samples were drawn using NUTS(diag_e) at Mon May 13 10:23:09 2019.
For each parameter, n_eff is a crude measure of effective sample size,
and Rhat is the potential scale reduction factor on split chains (at 
convergence, Rhat=1).
#+end_example
...
Warning messages:
1: There were 130 divergent transitions after warmup. Increasing adapt_delta above 0.8 may help. See
http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup 
2: There were 29 transitions after warmup that exceeded the maximum treedepth. Increase max_treedepth above 10. See
http://mc-stan.org/misc/warnings.html#maximum-treedepth-exceeded 
...
post-warmup draws per chain=250, total post-warmup draws=2000.
                  ...
                  mean se_mean     sd     2.5%      25%      50%     75%
intercept        63.12   15.45  32.48     6.88    41.54    60.36   99.71
first_degree      7.48    0.61   1.29     5.98     6.02     7.59    8.33
second_degree     1.99    0.01   0.01     1.97     1.98     1.99    2.00
sigma            15.12    4.76  10.15     3.92     4.16    14.88   21.47
lp__          -1456.41  200.83 409.77 -2006.60 -1779.47 -1594.54 -953.74
                97.5% n_eff Rhat
intercept      100.53     4 5.47
first_degree     9.74     4 5.32
second_degree    2.00     4 5.12
sigma           33.99     5 4.72
lp__          -951.94     4 8.67

Samples were drawn using NUTS(diag_e) at Mon May 13 11:16:36 2019.
For each parameter, n_eff is a crude measure of effective sample size,
and Rhat is the potential scale reduction factor on split chains (at 
convergence, Rhat=1).
#+end_example
...
post-warmup draws per chain=250, total post-warmup draws=2000.
                  ...
                  mean se_mean     sd     2.5%      25%      50%      75%
intercept       370.96    5.32 133.67   104.03   284.74   374.36   454.83
first_degree     -3.19    0.26   6.27   -14.95    -7.50    -3.26     1.00
second_degree     2.08    0.00   0.06     1.95     2.04     2.08     2.12
sigma          1025.21    1.04  32.52   964.19  1002.94  1023.27  1045.56
lp__          -3709.02    0.05   1.46 -3712.87 -3709.72 -3708.69 -3707.96
                 97.5% n_eff Rhat
intercept       628.72   631 1.02
first_degree      9.54   575 1.01
second_degree     2.20   627 1.01
sigma          1092.03   982 1.01
lp__          -3707.20   707 1.00

Samples were drawn using NUTS(diag_e) at Mon May 13 11:30:19 2019.
For each parameter, n_eff is a crude measure of effective sample size,
and Rhat is the potential scale reduction factor on split chains (at 
convergence, Rhat=1).
#+end_example
