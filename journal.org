...
generated quantities {
  int<lower=1,upper=2> comp[hosts];
  vector[hosts] alpha_pos;

  for (n in 1:hosts) {
    comp[n] = binomial_rng(1,theta);
    comp[n]=comp[n]+1;
    alpha_pos[n] = normal_rng(mu_alpha[comp[n]], sigma_alpha[comp[n]]);
  }
}
...
mnk=lapply(t_i, '[[', 1)
duration=lapply(t_i, '[[', 2)
dt = list(N=500,hosts=length(unique(tmp$cpu)),mnk=mnk,duration=duration,beta_mean=1e-05,beta_sd=1e-06,alpha_mean1=1e-10,alpha_mean2=7e-10,alpha_sd1=1e-11,alpha_sd2=1e-11,theta_mean=1e-05,theta_sd=1e-06,gamma_mean=1e-10,gamma_sd=1e-11)
