...
#+begin_src R :results output :session *R* :exports both
library(ggplot2)
frame=data.frame(alpha=parameters[1,],theta=parameters[2,],cpu=1:64)
#+end_src
...
visualisation graphs, except that theta's values are smaller. Now
we'll try to get two clusters of our values, especially for alpha.

#+begin_src R :results output graphics :file ./images/alpha_on_theta_clustered.png :exports both :width 600 :height 400 :session *R* 
means = kmeans(frame$alpha, 2, nstart = 50)
means$cluster = as.factor(means$cluster)
ggplot(frame, aes(theta,alpha, color = means$cluster)) + geom_point()+stat_ellipse(mapping = aes(theta,alpha))
#+end_src

#+RESULTS:
[[file:./images/alpha_on_theta_clustered.png]]

#+begin_src R :results output :session *R* :exports both
mean_alpha=c(means$centers)
...
: 1 7.818031e-11 4.181416e-12 2.190926e-13 4.699789e-14 0.125
: 2 6.712536e-11 4.181416e-12 2.190926e-13 4.699789e-14 0.875
