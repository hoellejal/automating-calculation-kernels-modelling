...
fit = sampling(sm,data=dt, iter=400, chains=3,init=initfunction)
#+end_src

#+RESULTS:
#+begin_example
Warning messages:
1: There were 285 divergent transitions after warmup. Increasing adapt_delta above 0.8 may help. See
http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup 
2: There were 311 transitions after warmup that exceeded the maximum treedepth. Increase max_treedepth above 10. See
http://mc-stan.org/misc/warnings.html#maximum-treedepth-exceeded 
3: Examine the pairs() plot to diagnose sampling problems
#+end_example

We had to reduce the number of chains and iterations because of the
time needed to make the fit.

#+begin_src R :results output :session *R* :exports both
extracted=rstan::extract(fit)
print(mean(extracted$intercept))
print(mean(extracted$mu))
print(mean(extracted$sigma))
print(mean(extracted$noise))
#+end_src
...
So without priors, the results found are not completely off, but they
...
And as expected with these results, the chains don't converge in the
trace. It isn't so surprising considering giving a prior is considered
one of the good practices in stan, and the more accurate the prior,
...
usual ones, and that's probably due to the init values given, that
guide stan in the right direction to start the fit.
...
We'll start with priors centered around 1e-01.
